...
doc = """
This is a normal games with 3x3 strategies. Two players are asked separately
...
class Constants(BaseConstants):
    name_in_url = 'normal_form_game_3x3'
    players_per_group = 2
    num_rounds = 30

    instructions_template = 'normal_form_game_3x3/Instructions.html'
...
class Subsession(BaseSubsession):
    def creating_session(self):
        ...
        strategies_P1 = [self.session.config['P1_strategy_1'], self.session.config['P1_strategy_2'], self.session.config['P1_strategy_3']]
        ...
        def create_random_payoffs(distribution, center, width):
            if distribution == "Normal":
                random_payoffs = random.normal(center, width, size=[2,3,3]) #8 is the number of payoffs for 2 players game ,2x2 startgies
        ...
        if (set_custom_random == 'False') * (user_distribution == 'False') * (user_width == 0) *(user_center == 0):
            payoffs_P1 = [[self.session.config['P1_payoff_P1S1_P2S1'], self.session.config['P1_payoff_P1S1_P2S2'], self.session.config['P1_payoff_P1S1_P2S3']],
                      ...
                      [self.session.config['P1_payoff_P1S3_P2S1'], self.session.config['P1_payoff_P1S3_P2S2'], self.session.config['P1_payoff_P1S3_P2S3']]]
            payoffs_P2 = [[self.session.config['P2_payoff_P2S1_P1S1'], self.session.config['P2_payoff_P2S1_P1S2'], self.session.config['P2_payoff_P2S1_P1S3']],
                      [self.session.config['P2_payoff_P2S2_P1S1'], self.session.config['P2_payoff_P2S2_P1S2'], self.session.config['P2_payoff_P2S2_P1S3']],
                      [self.session.config['P2_payoff_P2S3_P1S1'], self.session.config['P2_payoff_P2S3_P1S2'], self.session.config['P2_payoff_P2S3_P1S3']]]
